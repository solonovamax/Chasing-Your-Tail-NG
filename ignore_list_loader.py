...
class IgnoreListLoader:
    ...
    @classmethod
    def load_mac_list(cls, file_path: Path) -> List[str]:
        """
        ...
        try:
            ...
            if content.startswith('[') and content.endswith(']'):
                try:
                    ...
                    mac_list = cls.__parse_python_list(content, 'ignore_list')
            else:
    ...
    @classmethod
    def load_ssid_list(cls, file_path: Path) -> List[str]:
        """
        ...
        try:
            ...
            if content.startswith('[') and content.endswith(']'):
                try:
                    ...
                    # Fall back to Python list parsing
                    ssid_list = cls.__parse_python_list(content, 'non_alert_ssid_list')
            else:
                # Parse Python variable assignment
                ssid_list = cls.__parse_python_list(content, 'non_alert_ssid_list')
    ...
    @staticmethod
    def __parse_python_list(content: str, variable_name: str) -> List[str]:
        """
    ...
    @classmethod
    def save_mac_list(cls, mac_list: List[str], file_path: Path) -> None:
    ...
    @classmethod
    def save_ssid_list(cls, ssid_list: List[str], file_path: Path) -> None:
...
def load_ignore_lists(config: dict) -> tuple[List[str], List[str]]:
    ...
    """
    loader = IgnoreListLoader()

    # Load MAC ignore list
    mac_path = Path(config['paths']['ignore_lists']['mac'])
    mac_list = loader.load_mac_list(mac_path)

    # Load SSID ignore list  
    ssid_path = Path(config['paths']['ignore_lists']['ssid'])
